...
(function () {
  ...
  window.SmartChatbox = {
    ...
    isJobApplicationPage() {
      ...
      // Check URL
      const isApplicationUrl = applicationUrlPatterns.some((pattern) =>
        pattern.test(url),
      );
      ...
      const keywordMatches = applicationKeywords.filter((kw) =>
      ...
      if (
        pageTitle.includes("apply") ||
        pageTitle.includes("application") ||
        pageTitle.includes("career")
      )
        score += 15;

      console.log(
        `[SmartChatbox] Application detection score: ${score}/100 (URL: ${isApplicationUrl}, Keywords: ${keywordMatches})`,
      );
