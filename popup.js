...
function updateFullName() {
  const first = document.querySelector('[data-key="firstName"] .param-item-input')?.value || '';
  const last = document.querySelector('[data-key="lastName"] .param-item-input')?.value || '';
  const fullNameInput = document.querySelector('[data-key="fullName"] .param-item-input');
...
function runAutofillOnPage(data) {
  const MAPPINGS = {
    firstName: ['first_name','firstname','fname','given_name','givenname','first-name'],
    lastName: ['last_name','lastname','lname','surname','family_name','last-name'],
    fullName: ['full_name','fullname','name','your_name','candidate_name','legal_name'],
    email: ['email','e-mail','email_address','emailaddress','candidate_email'],
    phone: ['phone','telephone','tel','phone_number','mobile','cell','cellphone'],
    ...
    currentTitle: ['current_title','title','job_title','position','role','headline'],
    ...
    heardAbout: ['hear_about','heard_about','how_did_you_hear','source','referral'],
    salary: ['salary','salary_expectation','expected_salary','compensation'],
    startDate: ['start_date','availability','available_date','earliest_start','notice']
  };
  ...
  if (data.customParams) {
    data.customParams.forEach(param => {
      const key = param.key;
      const patterns = param.label.toLowerCase().replace(/[^a-z0-9]/g, '_').split('_').filter(Boolean);
  ...
  function detect(inp) {
    ...
    for (const [key, patterns] of Object.entries(MAPPINGS)) {
      for (const p of patterns) {
        if (attrs.includes(p.replace(/_/g, '')) || attrs.includes(p)) return key;
      }
    }
  ...
  toast.innerHTML = '<div style="display:flex;align-items:center;gap:12px">' +
    ...
    '<div><strong style="display:block;font-size:14px">' + count + ' fields filled</strong>' +
    '<span style="font-size:12px;color:#666">' + filled.slice(0,3).join(', ') + '</span></div></div>';
  toast.style.cssText = 'position:fixed;bottom:24px;right:24px;background:#fff;padding:16px 24px;' +
...
document.getElementById('confirmSectionBtn').addEventListener('click', async () => {
...
document.getElementById('confirmParamBtn').addEventListener('click', async () => {
...
function createParamItem(key, label, value = '') {
  ...
  // Add delete handler
  item.querySelector('.param-item-delete').addEventListener('click', async () => {
...
function renderCustomSections() {
  ...
  container.querySelectorAll('.param-section.custom-section').forEach(el => el.remove());
  ...
  customSections.forEach(section => {
    ...
    // Add delete handler
    sectionEl.querySelector('.delete-section-btn').addEventListener('click', async (e) => {
      e.stopPropagation();
    ...
    sectionParams.forEach(async param => {
      const data = await chrome.storage.sync.get([param.key]);
...
async function renderCustomParams() {
  ...
  const customSectionParams = customParams.filter(p => p.section === 'custom');
  ...
  const data = await chrome.storage.sync.get(customSectionParams.map(p => p.key));
...
document.getElementById('saveAiBtn').addEventListener('click', async () => {
  ...
  if (openaiKey) {
    ...
    try {
      ...
      showStatus('statusAi', '✓ Success! API key works. Settings saved.', 'success');
      
    } catch (err) {
      console.error('API key test failed:', err);
      let errorMsg = 'Problem with key: ';
      if (err.message.includes('Invalid API key') || err.message.includes('Incorrect API key')) {
...
document.getElementById('pdfFileInput').addEventListener('change', async (e) => {
  ...
  try {
    ...
    // Save both
    await chrome.storage.sync.set({ cvContent: allText, userContext: context });
    
    showStatus('statusAi', `✓ Extracted ${allText.length} chars from ${files.length} file${files.length > 1 ? 's' : ''} (${totalPages} pages). Both fields updated!`, 'success');
  } catch (err) {
...
function generateContextFromCV(cvText) {
  ...
  const achievements = cvText.match(/\d+%|\$[\d,]+|\d+\+|\d+ years?|\d+ months?/gi) || [];
  if (achievements.length > 0) {
    context.push(`Key metrics from CV: ${[...new Set(achievements)].slice(0, 5).join(', ')}`);
  }
  ...
  // Look for education
  const eduMatch = cvText.match(/(bachelor|master|mba|phd|degree|university|college)[^\n]*/gi);
  if (eduMatch) {
    context.push(`Education: ${eduMatch[0].substring(0, 150)}`);
  }
  
  // Look for job titles
  const titles = cvText.match(/(manager|director|engineer|developer|analyst|consultant|specialist|lead|head|vp|ceo|cto|founder)[^\n]*/gi);
  if (titles) {
    context.push(`Roles: ${[...new Set(titles)].slice(0, 3).join('; ').substring(0, 200)}`);
  }
